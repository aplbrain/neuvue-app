...
def create_stats_table(pending_tasks, closed_tasks):
    ...
    all_user_tasks = all_user_tasks[
        ~all_user_tasks.index.duplicated(keep="first")
    ...
    frames = [
        (
            daily_changelog_items,
            "closed",
            all_user_tasks[all_user_tasks.closed >= twentyFour_hrs_ago],
        ),
        (
            daily_changelog_items,
            "created",
            all_user_tasks[all_user_tasks.created >= twentyFour_hrs_ago],
        ),
        (
            full_changelog_items,
            "closed",
    ...
    for changelog, status, df in frames:
        for namespace, namespace_df in df.groupby("namespace"):
            ...
            with warnings.catch_warnings():
                ...
                average_event_time = (
                    (m + (namespace_df[status] - m)).mean().to_pydatetime()
                )
                changelog.append((average_event_time, n_tasks, status, namespace))

    # Sort changelogs by datetime
    daily_changelog_items = sorted(
        daily_changelog_items, key=lambda x: x[0], reverse=True
    )
    full_changelog_items = sorted(
        full_changelog_items, key=lambda x: x[0], reverse=True
    )
    ...
    return generate_changelog_text(daily_changelog_items), generate_changelog_text(
        full_changelog_items
    )
