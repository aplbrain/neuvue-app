...
def _get_nx_graph_image(nx_graph):
    def networkx_to_graphViz(nx_graph):
        ...
        timestamps = nx.get_node_attributes(nx_graph, "timestamp")
        operation_ids = nx.get_node_attributes(nx_graph, "operation_id")
        for node in nx_graph.nodes():
            label_str = str(node)
            label_str += '\n' + datetime.fromtimestamp(timestamps.get(node)).strftime('%Y-%m-%d') if timestamps.get(node) else ''
            label_str +=  '\n id: ' + str(operation_ids.get(node)) if operation_ids.get(node) else ''
