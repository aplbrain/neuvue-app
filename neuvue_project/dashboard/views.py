...
class DashboardView(View, LoginRequiredMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if display_name and group:
            ...
            return redirect(
                reverse("dashboard", kwargs={"namespace": namespace, "group": group})
            )
        elif username:
...
class DashboardNamespaceView(View, LoginRequiredMixin):
    def get(self, request, group=None, namespace=None, *args, **kwargs):
        ...
        context["display_name"] = Namespaces.objects.get(
            namespace=namespace
        ).display_name
        context["table"] = table
    ...
    def _generate_table_and_counts(self, namespace: str, users: List):
        ...
        if users:  # case if `group` of users was selected
            ...
            users = list(
                User.objects.all().values_list("username", flat=True).distinct()
            )  # retrieve all usernames

        task_df = client.get_tasks(sieve=sieve, select=["assignee", "status", "closed"])
...
class ReportView(View, LoginRequiredMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        decision_namespaces = [
            x.namespace
            for x in Namespaces.objects.filter(
                submission_method__in=list(button_sets)
            ).all()
        ]
        ...
        if start_field == end_field:
            sieve[start_field] = {"$gt": start_dt, "$lt": end_dt}
        else:
            sieve[start_field] = {"$gt": start_dt}
            sieve[end_field] = {"$lt": end_dt}
        ...
        for assignee, assignee_df in task_df.groupby("assignee"):
            ...
            avg_closed_duration = str(
                round(
                    assignee_df[assignee_df["status"] == "closed"]["duration"].mean()
                    / 60,
                    2,
                )
            )
            ...
            user_metrics = [assignee, total_duration, avg_closed_duration, avg_duration, avg_edits]
        ...
        fields = {"created": "Created By", "opened": "Opened By", "closed": "Closed By"}
...
class UserNamespaceView(View, LoginRequiredMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Retrieve valid tasks
        namespace = (
            Namespaces.objects.get(display_name=display_name).namespace
            if display_name
            else ""
        )
        ...
        if display_name:
            ...
            for assignee, assignee_df in task_df.groupby("assignee"):
                ...
                avg_closed_duration = str(
                    round(
                        assignee_df[assignee_df["status"] == "closed"][
                            "duration"
                        ].mean()
                        / 60,
                        2,
            ...
            for namespace, namespace_df in task_df.groupby("namespace"):
                ...
                avg_closed_duration = str(
                    round(
                        namespace_df[namespace_df["status"] == "closed"][
                            "duration"
                        ].mean()
                        / 60,
                        2,
                ...
                for status in status_states:
                    number_of_tasks = len(
                        namespace_df[namespace_df["status"] == status]
                    )
